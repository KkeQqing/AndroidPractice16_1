...
public class UdpVideoChat {
    ...
    // 后台发送线程池，彻底避免 NetworkOnMainThreadException
    ...
    private void sendFrameFromImage(ImageProxy imageProxy) {
        Log.d("VideoChat", "获取到一帧，准备发送");
        Image image = imageProxy.getImage();
        if (image == null) return;
        try {
            byte[] jpegData = compressYUV420ToJpeg(image);
            if (jpegData != null) {
                Log.d("VideoChat", "JPEG大小: " + jpegData.length);
                // 在后台线程执行网络发送，关键修复点
                sendExecutor.execute(() -> handler.sendFrame(jpegData));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private byte[] compressYUV420ToJpeg(Image image) {
        ...
        yuvImage.compressToJpeg(new Rect(0, 0, width, height), 50, out);  // 质量50
    ...
    public void stop() {
        ...
        sendExecutor.shutdownNow();  // 释放发送线程
    }
}